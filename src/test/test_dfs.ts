...
import { dfs, dfsVertex, dfsGraph } from '../algos/traversal';
...
describe('Traversal', () => {
  ...
  it('#DFS - Tree', () => {
    ...
    const tree: Array<Vertex> = dfsVertex(g, vertices[0]);
    expect(tree.map(node => (node as Vertex).id)).to.deep.equal([0, 2, 1, 3, 4]);
  });
  it('#DFS - Forest', () => {
    expect(vertices.length).to.equal(5);
    expect(edges.length).to.equal(5);
    const forest: Array<Array<Vertex>> = dfsGraph(g);
    expect(forest[0].map(node => (node as Vertex).id)).to.deep.equal([0, 2, 1, 3, 4]);
  });
});
